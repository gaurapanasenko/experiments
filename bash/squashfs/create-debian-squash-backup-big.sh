...
search --set=root --file /Debian_AF9sJTzIyz
...
cp $DIR/scratch/grub.cfg $DIR/image/grub.cfg

touch $DIR/image/Debian_AF9sJTzIyz
...
xorriso \
    ...
    -output "$DIR/debian-custom.iso" \
    -graft-points \
        "$DIR/image" \
        /boot/grub/bios.img=$DIR/scratch/bios.img

chmod 644 $DIR/debian-custom.iso
